<template>
  <Form ref="formRef" :model="formState" layout="vertical">
    <Form.Item
      ...
      :rules="[{ required: true, message: '请选择遥测数据Keys!' }]"
      ...
      <div class="p-2">
        ...
        <template v-if="formState.useMetadataIntervalPatterns == true">
          <Form.Item
            ...
            :rules="[{ required: true, message: '开始间隔必填!' }]"
          >
      ...
      <div class="p-2">
        ...
        <Alert type="success">
          <template #message>
            <span v-if="formState.fetchMode == 'FIRST'">
              如果选择提取策略'First'规则节点将检索到最接近提取间隔开始的遥测数据。</span
            >
            <span v-else-if="formState.fetchMode == 'LAST'">
              如果选择了提取模式'Last'规则节点将检索到距离提取间隔结束最近的遥测数据。</span
            >
            <span v-else="formState.fetchMode == 'ALL'">
              如果选择了提取模式'All'规则节点将使用可配置的查询参数从提取间隔中检索遥测数据。</span
            >
          </template>
        </Alert>
        <template v-if="formState.fetchMode == 'ALL'">
          ...
          <Form.Item
            label="Limit"
            name="limit"
            :rules="[{ required: true, message: 'Limit必填!' }]"
