  ...
  <Form ref="formRef" :model="formState" layout="vertical">
    <div class="border border-solid border-neutral-300 rounded-md py-2 px-3 mb-4">
      ...
      <div class="p-2">
        ...
        >
          <Checkbox v-model:checked="formState.deviceRelationsQuery.fetchLastLevelOnly"> 仅获取最后一级关联 </Checkbox>
        </Form.Item>
    ...
    <div class="border border-solid border-neutral-300 rounded-md py-2 px-3 mb-4">
      ...
      <div class="p-2">
        ...
        <Form.Item name="getLatestValueWithTs" v-show="formState.latestTsKeyNames.length > 0">
          <Checkbox v-model:checked="formState.getLatestValueWithTs"> 获取带有时间戳的最后一条数据 </Checkbox>
        </Form.Item>
      </div>
    </div>
    ...
    <Form.Item name="tellFailureIfAbsent" help="如果至少有一个选定的密钥不存在，则出消息将报告“Failure”">
      <div class="border border-solid border-neutral-300 rounded-md px-4 py-2">
        <Checkbox v-model:checked="formState.tellFailureIfAbsent"> 如果缺少任何属性，则告知失败 </Checkbox>
      </div>
    </Form.Item>
  </Form>
</template>
