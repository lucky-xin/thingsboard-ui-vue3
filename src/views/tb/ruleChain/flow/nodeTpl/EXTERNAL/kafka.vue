<template>
  <Form ref="formRef" :model="formState" layout="vertical">
    ...
    <Form.Item
      label="Number of acknowledgements"
      name="acks"
    ...
    <Form.Item
      ...
      name="keySerializer"
      :rules="[{ required: true, message: '请输入 key serializer' }]"
    >
    ...
    <Form.Item label="Other Properties" name="otherProperties" :rules="[{ validator: validatorOtherProperties }]">
      <Table class="properties-table">
        ...
        <tr class="content" v-for="(property, index) in otherPropertyList" :key="index">
          <td class="py-2 px-4"> <Input v-model:value="property.key" placeholder="输入Key" /> </td>
          <td> <Input v-model:value="property.value" placeholder="输入Value" /> </td>
          <td>
    ...
    <Form.Item
      ...
      help="如果选中，消息元数据中的键值对将作为具有预定义字符集编码的字节数组添加到传出记录头中。"
    >
