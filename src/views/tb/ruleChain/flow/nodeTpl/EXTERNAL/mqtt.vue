<template>
  <Form ref="formRef" :model="formState" layout="vertical">
    <Form.Item
      label="Topic pattern"
      name="topicPattern"
      :rules="[{ required: true, message: '请输入Topic pattern!' }]"
    ...
    <Form.Item
      ...
      help="可选择的当明确指定“Client ID”时应用。如果选中，则Server ID将作为后缀添加到Client ID中。有助于避免平台在微服务模式下运行时出现故障。"
    >
      <Checkbox v-model:checked="formState.appendClientIdSuffix"> 将Server ID 作为后缀添加到Client ID </Checkbox>
    </Form.Item>
    <Form.Item name="cleanSession">
      <Checkbox v-model:checked="formState.cleanSession"> Clean Session </Checkbox>
    </Form.Item>
    <Form.Item name="retainedMessage">
      <Checkbox v-model:checked="formState.retainedMessage"> Retained </Checkbox>
    </Form.Item>
    <Form.Item name="ssl">
      <Checkbox v-model:checked="formState.ssl"> Enable SSL </Checkbox>
    </Form.Item>
