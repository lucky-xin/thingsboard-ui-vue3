<template>
  <Form ref="formRef" :model="formState" layout="vertical">
    ...
    <Form.Item
      ...
      name="requestMethod"
      :rules="[{ required: true, message: 'Request method 必选!' }]"
    >
    ...
    <Form.Item
      ...
      help="如果选中，请求正文消息负载将从JSON字符串转换为纯文本，例如msg=“Hello，\t\t”world\“”将解析为Hello，”world“"
    >
    ...
    <Form.Item label="Http请求头" name="headers" :rules="[{ validator: validatorHeader }]">
      ...
      <Table class="header-table" aria-label="请求头表格">
        ...
        <tr class="content" v-for="(header, index) in headerList" :key="index">
          <td class="py-2 px-4"> <Input v-model:value="header.key" placeholder="输入Header" /> </td>
          <td> <Input v-model:value="header.value" placeholder="输入Value" /> </td>
          <td>
