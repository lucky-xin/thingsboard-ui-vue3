import { describe, it, expect } from 'vitest';
import {
  SIDE_BAR_MINI_WIDTH,
  SIDE_BAR_SHOW_TIT_MINI_WIDTH,
  ContentEnum,
  ThemeEnum,
  SettingButtonPositionEnum,
  SessionTimeoutProcessingEnum,
  PermissionModeEnum,
  RouterTransitionEnum
} from '/@/enums/appEnum';

describe('enums/appEnum', () => {
  describe('Constants', () => {
    it('should have correct SIDE_BAR_MINI_WIDTH value', () => {
      expect(SIDE_BAR_MINI_WIDTH).toBe(48);
      expect(typeof SIDE_BAR_MINI_WIDTH).toBe('number');
    });

    it('should have correct SIDE_BAR_SHOW_TIT_MINI_WIDTH value', () => {
      expect(SIDE_BAR_SHOW_TIT_MINI_WIDTH).toBe(80);
      expect(typeof SIDE_BAR_SHOW_TIT_MINI_WIDTH).toBe('number');
    });
  });

  describe('ContentEnum', () => {
    it('should have correct FULL value', () => {
      expect(ContentEnum.FULL).toBe('full');
    });

    it('should have correct FIXED value', () => {
      expect(ContentEnum.FIXED).toBe('fixed');
    });

    it('should have all expected values', () => {
      const values = Object.values(ContentEnum);
      expect(values).toContain('full');
      expect(values).toContain('fixed');
      expect(values).toHaveLength(2);
    });
  });

  describe('ThemeEnum', () => {
    it('should have correct DARK value', () => {
      expect(ThemeEnum.DARK).toBe('dark');
    });

    it('should have correct LIGHT value', () => {
      expect(ThemeEnum.LIGHT).toBe('light');
    });

    it('should have all expected values', () => {
      const values = Object.values(ThemeEnum);
      expect(values).toContain('dark');
      expect(values).toContain('light');
      expect(values).toHaveLength(2);
    });
  });

  describe('SettingButtonPositionEnum', () => {
    it('should have correct AUTO value', () => {
      expect(SettingButtonPositionEnum.AUTO).toBe('auto');
    });

    it('should have correct HEADER value', () => {
      expect(SettingButtonPositionEnum.HEADER).toBe('header');
    });

    it('should have correct FIXED value', () => {
      expect(SettingButtonPositionEnum.FIXED).toBe('fixed');
    });

    it('should have all expected values', () => {
      const values = Object.values(SettingButtonPositionEnum);
      expect(values).toContain('auto');
      expect(values).toContain('header');
      expect(values).toContain('fixed');
      expect(values).toHaveLength(3);
    });
  });

  describe('SessionTimeoutProcessingEnum', () => {
    it('should have correct ROUTE_JUMP value', () => {
      expect(SessionTimeoutProcessingEnum.ROUTE_JUMP).toBe(0);
    });

    it('should have correct PAGE_COVERAGE value', () => {
      expect(SessionTimeoutProcessingEnum.PAGE_COVERAGE).toBe(1);
    });

    it('should have all expected values', () => {
      const values = Object.values(SessionTimeoutProcessingEnum);
      expect(values).toContain(0);
      expect(values).toContain(1);
      expect(values).toHaveLength(4); // includes string keys
    });
  });

  describe('PermissionModeEnum', () => {
    it('should have correct ROLE value', () => {
      expect(PermissionModeEnum.ROLE).toBe('ROLE');
    });

    it('should have correct BACK value', () => {
      expect(PermissionModeEnum.BACK).toBe('BACK');
    });

    it('should have correct ROUTE_MAPPING value', () => {
      expect(PermissionModeEnum.ROUTE_MAPPING).toBe('ROUTE_MAPPING');
    });

    it('should have all expected values', () => {
      const values = Object.values(PermissionModeEnum);
      expect(values).toContain('ROLE');
      expect(values).toContain('BACK');
      expect(values).toContain('ROUTE_MAPPING');
      expect(values).toHaveLength(3);
    });
  });

  describe('RouterTransitionEnum', () => {
    it('should have correct ZOOM_FADE value', () => {
      expect(RouterTransitionEnum.ZOOM_FADE).toBe('zoom-fade');
    });

    it('should have correct ZOOM_OUT value', () => {
      expect(RouterTransitionEnum.ZOOM_OUT).toBe('zoom-out');
    });

    it('should have correct FADE_SIDE value', () => {
      expect(RouterTransitionEnum.FADE_SIDE).toBe('fade-slide');
    });

    it('should have correct FADE value', () => {
      expect(RouterTransitionEnum.FADE).toBe('fade');
    });

    it('should have correct FADE_BOTTOM value', () => {
      expect(RouterTransitionEnum.FADE_BOTTOM).toBe('fade-bottom');
    });

    it('should have correct FADE_SCALE value', () => {
      expect(RouterTransitionEnum.FADE_SCALE).toBe('fade-scale');
    });

    it('should have all expected values', () => {
      const values = Object.values(RouterTransitionEnum);
      expect(values).toContain('zoom-fade');
      expect(values).toContain('zoom-out');
      expect(values).toContain('fade-slide');
      expect(values).toContain('fade');
      expect(values).toContain('fade-bottom');
      expect(values).toContain('fade-scale');
      expect(values).toHaveLength(6);
    });
  });
});