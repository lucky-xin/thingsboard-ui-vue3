import { describe, it, expect, vi, beforeEach } from 'vitest';
import { useEventListener } from '/@/hooks/event/useEventListener';

// Mock Vue composition functions
vi.mock('vue', () => ({
  ref: vi.fn((value) => ({ value })),
  watch: vi.fn(),
  unref: vi.fn((val) => (val && typeof val === 'object' && 'value' in val ? val.value : val)),
}));

// Mock VueUse functions
vi.mock('@vueuse/core', () => ({
  useThrottleFn: vi.fn((fn) => fn),
  useDebounceFn: vi.fn((fn) => fn),
}));

import { ref, watch, unref } from 'vue';
import { useThrottleFn, useDebounceFn } from '@vueuse/core';

describe('hooks/event/useEventListener', () => {
  let mockElement: any;
  let mockAddEventListener: any;
  let mockRemoveEventListener: any;

  beforeEach(() => {
    vi.clearAllMocks();
    
    mockAddEventListener = vi.fn();
    mockRemoveEventListener = vi.fn();
    
    mockElement = {
      addEventListener: mockAddEventListener,
      removeEventListener: mockRemoveEventListener,
    };

    // Setup default mocks
    (ref as any).mockImplementation((value) => ({ value }));
    (unref as any).mockImplementation((val) => 
      val && typeof val === 'object' && 'value' in val ? val.value : val
    );
    (watch as any).mockImplementation((source, callback, options) => {
      if (options?.immediate) {
        // Call the callback immediately for testing
        const cleanup = vi.fn();
        callback(source.value || source, undefined, cleanup);
      }
      return vi.fn(); // Return cleanup function
    });
  });

  it('should return removeEvent function', () => {
    const result = useEventListener({
      el: mockElement,
      name: 'click',
      listener: vi.fn(),
    });

    expect(result).toHaveProperty('removeEvent');
    expect(typeof result.removeEvent).toBe('function');
  });

  it('should use window as default element', () => {
    // Mock window
    Object.defineProperty(global, 'window', {
      value: {
        addEventListener: vi.fn(),
        removeEventListener: vi.fn(),
      },
      writable: true,
    });

    const listener = vi.fn();
    useEventListener({
      name: 'resize',
      listener,
    });

    expect(ref).toHaveBeenCalled();
    expect(watch).toHaveBeenCalled();
  });

  it('should add event listener when element is provided', () => {
    const listener = vi.fn();
    
    useEventListener({
      el: mockElement,
      name: 'click',
      listener,
      options: { passive: true },
    });

    expect(mockAddEventListener).toHaveBeenCalledWith('click', expect.any(Function), { passive: true });
  });

  it('should use debounced listener when isDebounce is true', () => {
    const listener = vi.fn();
    const mockDebouncedFn = vi.fn();
    (useDebounceFn as any).mockReturnValue(mockDebouncedFn);

    useEventListener({
      el: mockElement,
      name: 'scroll',
      listener,
      isDebounce: true,
      wait: 100,
    });

    expect(useDebounceFn).toHaveBeenCalledWith(listener, 100);
  });

  it('should use throttled listener when isDebounce is false', () => {
    const listener = vi.fn();
    const mockThrottledFn = vi.fn();
    (useThrottleFn as any).mockReturnValue(mockThrottledFn);

    useEventListener({
      el: mockElement,
      name: 'scroll',
      listener,
      isDebounce: false,
      wait: 50,
    });

    expect(useThrottleFn).toHaveBeenCalledWith(listener, 50);
  });

  it('should use original listener when wait is 0', () => {
    const listener = vi.fn();

    useEventListener({
      el: mockElement,
      name: 'click',
      listener,
      wait: 0,
    });

    expect(useDebounceFn).toHaveBeenCalledWith(listener, 0);
    expect(mockAddEventListener).toHaveBeenCalled();
  });

  it('should remove event listener when removeEvent is called', () => {
    const listener = vi.fn();
    
    const { removeEvent } = useEventListener({
      el: mockElement,
      name: 'click',
      listener,
    });

    removeEvent();

    expect(mockRemoveEventListener).toHaveBeenCalledWith('click', expect.any(Function), undefined);
  });

  it('should watch element changes and add/remove listeners accordingly', () => {
    const listener = vi.fn();
    const mockWatchCallback = vi.fn();
    
    (watch as any).mockImplementation((source, callback) => {
      mockWatchCallback.mockImplementation(callback);
      return vi.fn();
    });

    useEventListener({
      el: mockElement,
      name: 'click',
      listener,
    });

    expect(watch).toHaveBeenCalled();
    
    // Test the watch callback
    const watchCall = (watch as any).mock.calls[0];
    expect(watchCall[2]).toEqual({ immediate: true });
  });

  it('should handle autoRemove option correctly', () => {
    const listener = vi.fn();
    
    useEventListener({
      el: mockElement,
      name: 'click',
      listener,
      autoRemove: false,
    });

    expect(watch).toHaveBeenCalled();
  });

  it('should handle null/undefined element', () => {
    const result = useEventListener({
      el: null,
      name: 'click',
      listener: vi.fn(),
    });

    expect(result.removeEvent).toBeTruthy();
    expect(typeof result.removeEvent).toBe('function');
  });

  it('should clean up on element value change', () => {
    const listener = vi.fn();
    let cleanupFn: any;
    
    (watch as any).mockImplementation((source, callback, options) => {
      if (options?.immediate) {
        cleanupFn = vi.fn();
        callback(mockElement, undefined, cleanupFn);
      }
      return vi.fn();
    });

    useEventListener({
      el: mockElement,
      name: 'click',
      listener,
      autoRemove: true,
    });

    // Simulate cleanup
    if (cleanupFn) {
      cleanupFn();
    }
  });

  it('should handle different event listener options', () => {
    const listener = vi.fn();
    const options = { 
      capture: true, 
      passive: true, 
      once: true 
    };

    useEventListener({
      el: mockElement,
      name: 'touchstart',
      listener,
      options,
    });

    expect(mockAddEventListener).toHaveBeenCalledWith('touchstart', expect.any(Function), options);
  });

  it('should use default wait time when not specified', () => {
    const listener = vi.fn();

    useEventListener({
      el: mockElement,
      name: 'input',
      listener,
    });

    expect(useDebounceFn).toHaveBeenCalledWith(listener, 80);
  });

  it('should handle reactive element ref', () => {
    const elementRef = { value: mockElement };
    const listener = vi.fn();

    (ref as any).mockReturnValue(elementRef);

    useEventListener({
      el: elementRef,
      name: 'click',
      listener,
    });

    expect(ref).toHaveBeenCalledWith(elementRef);
    expect(watch).toHaveBeenCalled();
  });
});