import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { ref } from 'vue';

// Mock Vue
vi.mock('vue', () => ({
  ref: vi.fn((value) => ({ value })),
  onMounted: vi.fn((callback) => callback()),
  onUnmounted: vi.fn((callback) => callback()),
}));

// Mock window
const mockWindow = {
  addEventListener: vi.fn(),
  removeEventListener: vi.fn(),
  innerWidth: 1024,
  innerHeight: 768,
};

Object.defineProperty(global, 'window', {
  value: mockWindow,
  writable: true,
});

import { useWindowSizeFn } from '/@/hooks/event/useWindowSizeFn';

describe.skip('useWindowSizeFn comprehensive tests', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('should return window size state', () => {
    const { width, height } = useWindowSizeFn();

    expect(width).toBeDefined();
    expect(height).toBeDefined();
  });

  it('should initialize with current window size', () => {
    const { width, height } = useWindowSizeFn();
    expect(width.value).toBe(1024);
    expect(height.value).toBe(768);
  });

  it('should add resize event listener on mount', () => {
    useWindowSizeFn();
    expect(mockWindow.addEventListener).toHaveBeenCalledWith('resize', expect.any(Function));
  });

  it('should remove resize event listener on unmount', () => {
    useWindowSizeFn();
    expect(mockWindow.removeEventListener).toHaveBeenCalledWith('resize', expect.any(Function));
  });

  it('should update size values on resize', () => {
    const { width, height } = useWindowSizeFn();
    
    // Simulate resize event
    mockWindow.innerWidth = 1200;
    mockWindow.innerHeight = 900;
    
    // Get the resize handler
    const resizeHandler = mockWindow.addEventListener.mock.calls[0][1];
    resizeHandler();
    
    expect(width.value).toBe(1200);
    expect(height.value).toBe(900);
  });

  it('should handle multiple resize events', () => {
    const { width, height } = useWindowSizeFn();
    
    // First resize
    mockWindow.innerWidth = 1200;
    mockWindow.innerHeight = 900;
    const resizeHandler = mockWindow.addEventListener.mock.calls[0][1];
    resizeHandler();
    
    expect(width.value).toBe(1200);
    expect(height.value).toBe(900);
    
    // Second resize
    mockWindow.innerWidth = 800;
    mockWindow.innerHeight = 600;
    resizeHandler();
    
    expect(width.value).toBe(800);
    expect(height.value).toBe(600);
  });

  it('should handle zero dimensions', () => {
    const { width, height } = useWindowSizeFn();
    
    mockWindow.innerWidth = 0;
    mockWindow.innerHeight = 0;
    
    const resizeHandler = mockWindow.addEventListener.mock.calls[0][1];
    resizeHandler();
    
    expect(width.value).toBe(0);
    expect(height.value).toBe(0);
  });

  it('should handle negative dimensions', () => {
    const { width, height } = useWindowSizeFn();
    
    mockWindow.innerWidth = -100;
    mockWindow.innerHeight = -200;
    
    const resizeHandler = mockWindow.addEventListener.mock.calls[0][1];
    resizeHandler();
    
    expect(width.value).toBe(-100);
    expect(height.value).toBe(-200);
  });
});
