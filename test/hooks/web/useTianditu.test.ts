import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { defHttp } from '/@/utils/http/axios';
import { useTianditu } from '/@/hooks/web/useTianditu';

// Mock Vue composition API
vi.mock('vue', () => ({
  onMounted: vi.fn((callback) => callback()),
  onUnmounted: vi.fn((callback) => {
    (global as any).onUnmountedCallback = callback;
  }),
  ref: vi.fn((value) => ({ value })),
}));

// Mock HTTP client
vi.mock('/@/utils/http/axios', () => ({
  defHttp: {
    get: vi.fn(),
  },
}));

// Mock DOM APIs
const mockScript = {
  type: '',
  onerror: null as any,
  onload: null as any,
  src: '',
  remove: vi.fn(),
};

const mockDocument = {
  createElement: vi.fn(() => mockScript),
  head: {
    appendChild: vi.fn(),
  },
};

describe('hooks/web/useTianditu', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    global.document = mockDocument as any;
    global.window = {} as any;
    mockScript.type = '';
    mockScript.src = '';
    mockScript.onerror = null;
    mockScript.onload = null;
  });

  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('should initialize with correct default values', () => {
    const result = useTianditu('test-tk');

    expect(result.error.value).toBe(false);
    expect(result.success.value).toBe(false);
    expect(result.T.value).toBeUndefined();
    expect(result.geocoder).toBeInstanceOf(Function);
    expect(result.toPromise).toBeInstanceOf(Function);
  });

  it('should create script element with correct attributes', () => {
    useTianditu('test-tk');

    expect(mockDocument.createElement).toHaveBeenCalledWith('script');
    expect(mockScript.type).toBe('text/javascript');
    expect(mockScript.src).toBe('https://api.tianditu.gov.cn/api?v=4.0&tk=test-tk');
    expect(mockDocument.head.appendChild).toHaveBeenCalledWith(mockScript);
  });

  it('should handle successful script load', async () => {
    const mockTianditu = { test: 'value' };
    global.window = { T: mockTianditu } as any;

    const result = useTianditu('test-tk');
    const promise = result.toPromise();

    // Simulate successful load
    mockScript.onload();

    const resolvedValue = await promise;

    expect(result.success.value).toBe(true);
    expect(result.error.value).toBe(false);
    expect(result.T.value).toBe(mockTianditu);
    expect(resolvedValue).toBe(mockTianditu);
  });

  it('should handle script error', async () => {
    const result = useTianditu('test-tk');
    const promise = result.toPromise();

    // Simulate script error
    const mockError = new Error('Script load failed');
    mockScript.onerror(mockError);

    await expect(promise).rejects.toThrow('Script load failed');
    expect(result.success.value).toBe(false);
    expect(result.error.value).toBe(true);
  });

  it('should remove script on unmount', () => {
    useTianditu('test-tk');

    // Simulate unmount
    if ((global as any).onUnmountedCallback) {
      (global as any).onUnmountedCallback();
    }

    expect(mockScript.remove).toHaveBeenCalled();
  });

  it('should handle unmount without script', () => {
    const result = useTianditu('test-tk');

    // Simulate unmount
    if ((global as any).onUnmountedCallback) {
      (global as any).onUnmountedCallback();
    }

    // Should not throw error
    expect(mockScript.remove).toHaveBeenCalled();
  });

  it('should geocode successfully', async () => {
    const mockLocation = {
      keyWord: 'Beijing',
      lat: 39.9042,
      lon: 116.4074,
      level: 'city',
      score: 95,
    };

     vi.mocked(defHttp.get).mockResolvedValue({
      msg: 'ok',
      location: mockLocation,
    });

    const result = useTianditu('test-tk');

    const location = await result.geocoder('Beijing');

     expect(vi.mocked(defHttp.get)).toHaveBeenCalledWith(
      {
        url: 'https://api.tianditu.gov.cn/geocoder',
        params: {
          ds: '{"keyWord":"Beijing"}',
          tk: 'test-tk',
        },
      },
      { withToken: false }
    );

    expect(location).toEqual(mockLocation);
  });

  it('should return null when geocoding fails', async () => {
     vi.mocked(defHttp.get).mockResolvedValue({
      msg: 'error',
      location: null,
    });

    const result = useTianditu('test-tk');

    const location = await result.geocoder('InvalidLocation');

    expect(location).toBeNull();
  });

  it('should handle geocoding HTTP error', async () => {
     vi.mocked(defHttp.get).mockRejectedValue(new Error('Network error'));

    const result = useTianditu('test-tk');

    await expect(result.geocoder('Beijing')).rejects.toThrow('Network error');
  });

  it('should construct correct geocoding parameters', async () => {
     vi.mocked(defHttp.get).mockResolvedValue({
      msg: 'ok',
      location: {},
    });

    const result = useTianditu('test-tk');

    await result.geocoder('Test City');

     expect(vi.mocked(defHttp.get)).toHaveBeenCalledWith(
      {
        url: 'https://api.tianditu.gov.cn/geocoder',
        params: {
          ds: '{"keyWord":"Test City"}',
          tk: 'test-tk',
        },
      },
      { withToken: false }
    );
  });

  it('should handle empty keyword in geocoding', async () => {
     vi.mocked(defHttp.get).mockResolvedValue({
      msg: 'ok',
      location: {},
    });

    const result = useTianditu('test-tk');

    await result.geocoder('');

     expect(vi.mocked(defHttp.get)).toHaveBeenCalledWith(
      {
        url: 'https://api.tianditu.gov.cn/geocoder',
        params: {
          ds: '{"keyWord":""}',
          tk: 'test-tk',
        },
      },
      { withToken: false }
    );
  });

  it('should handle special characters in geocoding', async () => {
     vi.mocked(defHttp.get).mockResolvedValue({
      msg: 'ok',
      location: {},
    });

    const result = useTianditu('test-tk');

    await result.geocoder('Beijing & Shanghai');

     expect(vi.mocked(defHttp.get)).toHaveBeenCalledWith(
      {
        url: 'https://api.tianditu.gov.cn/geocoder',
        params: {
          ds: '{"keyWord":"Beijing & Shanghai"}',
          tk: 'test-tk',
        },
      },
      { withToken: false }
    );
  });
});
