...
// 使用全局 setup 中提供的真实 Pinia 实例，不再在此处 mock '/@/store'

// Mock the settings
vi.mock('/@/settings/designSetting', async () => {
  const actual = await vi.importActual('/@/settings/designSetting');
  return {
    ...actual,
    darkMode: 'light',
  };
...
vi.mock('/@/utils/cache/persistent', async (importOriginal) => {
