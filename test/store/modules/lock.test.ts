import { describe, it, expect, vi, beforeEach } from 'vitest';
import { useLockStore } from '/@/store/modules/lock';
import { LOCK_INFO_KEY } from '/@/enums/cacheEnum';

// Mock the cache
vi.mock('/@/utils/cache/persistent', () => ({
  Persistent: {
    getLocal: vi.fn(),
    setLocal: vi.fn(),
    removeLocal: vi.fn(),
  },
}));

// Mock the user store
vi.mock('/@/store/modules/user', () => ({
  useUserStore: () => ({
    getUserInfo: {
      loginCode: 'testuser',
    },
    login: vi.fn().mockResolvedValue({ result: 'true' }),
  }),
}));

describe('store/modules/lock', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  afterEach(() => {
    vi.restoreAllMocks();
  });

  describe('useLockStore', () => {
    it('should create lock store', () => {
      const store = useLockStore();
      expect(store).toBeDefined();
    });

    it('should have initial state', () => {
      const store = useLockStore();
      expect(store.getLockInfo).toBeUndefined();
    });

    it('should set lock info', () => {
      const store = useLockStore();
      const lockInfo = { pwd: '123456', isLock: true };

      store.setLockInfo(lockInfo);

      expect(store.getLockInfo).toEqual(lockInfo);
    });

    it('should reset lock info', () => {
      const store = useLockStore();
      const lockInfo = { pwd: '123456', isLock: true };
      store.setLockInfo(lockInfo);

      store.resetLockInfo();

      expect(store.getLockInfo).toBeNull();
    });

    it('should unlock with correct password', async () => {
      const store = useLockStore();
      const password = '123456';
      const lockInfo = { pwd: password, isLock: true };
      store.setLockInfo(lockInfo);

      const result = await store.unLock(password);

      expect(result).toBe(true);
      expect(store.getLockInfo).toBeNull();
    });

    it('should unlock with login credentials', async () => {
      const store = useLockStore();
      const password = 'wrongpassword';
      const lockInfo = { pwd: '123456', isLock: true };
      store.setLockInfo(lockInfo);

      const result = await store.unLock(password);

      // This should return false since the password is wrong
      expect(result).toBe(false);
    });

    it('should fail to unlock with incorrect password', async () => {
      const store = useLockStore();
      const password = 'wrongpassword';
      const lockInfo = { pwd: '123456', isLock: true };
      store.setLockInfo(lockInfo);

      // Mock the user store module to return a user store with failing login
      vi.mock('/@/store/modules/user', () => {
        return {
          useUserStore: () => ({
            getUserInfo: {
              loginCode: 'testuser',
            },
            login: vi.fn().mockResolvedValue({ result: 'false' }),
          }),
        };
      });

      // Re-import the lock store to use the new mock
      const { useLockStore: newUseLockStore } = await import('/@/store/modules/lock');
      const newStore = newUseLockStore();
      newStore.setLockInfo(lockInfo);

      const result = await newStore.unLock(password);

      expect(result).toBe(false);
    });
  });
});
