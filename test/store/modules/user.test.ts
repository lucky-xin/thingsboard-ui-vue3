// Mock state management and global dependencies
vi.mock("/@/store", () => ({
  useAppStore: () => ({
    getTheme: vi.fn(() => "light"),
    setTheme: vi.fn(),
    locale: "en",
    setLocale: vi.fn()
  }),
  useUserStore: () => ({
    userInfo: { name: "Test User" },
    isLoggedIn: true
  })
}));

vi.mock("/@/hooks/setting/useLocale", () => ({
  useLocale: () => ({
    getLocale: vi.fn(() => ({ lang: "en" })),
    changeLocale: vi.fn(),
    t: vi.fn((key) => key)
  })
}));

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { useUserStore, useUserStoreWithOut, useEmitter } from '/@/store/modules/user';
import type { UserInfo, JwtPair } from '/#/store';
import { TOKEN_KEY, USER_INFO_KEY, SESSION_TIMEOUT_KEY, REFRESH_TOKEN_KEY, AUTHORITY_KEY } from '/@/enums/cacheEnum';
import { createPinia, setActivePinia } from 'pinia';

// Mock the store with proper Pinia instance
vi.mock('/@/store', async (importOriginal) => {
  const actual = await importOriginal();
  return {
    ...actual,
    store: { 
      // Mock the Pinia instance methods that might be used
      use: vi.fn(),
      _s: new Map(), // Mock the stores map
      _p: [], // Mock the plugins array
      install: vi.fn(),
      state: {
        value: {}
      },
      _e: {
        active: true,
        run: vi.fn((fn) => fn())
      }
    },
    useAppStore: () => ({
      getTheme: vi.fn(() => "light"),
      setTheme: vi.fn(),
      locale: "en",
      setLocale: vi.fn()
    }),
    useUserStore: () => ({
      userInfo: { name: "Test User" },
      isLoggedIn: true
    })
  };
});

// Mock the auth cache functions
vi.mock('/@/utils/auth', () => ({
  getAuthCache: vi.fn(),
  setAuthCache: vi.fn(),
}));

// Mock the login API
vi.mock('/@/api/tb/login', () => ({
  loginApi: vi.fn(),
  logoutApi: vi.fn(),
  userInfoApi: vi.fn(),
  LoginParams: {},
}));

// Mock the message hook
vi.mock('/@/hooks/web/useMessage', () => ({
  useMessage: () => ({
    showMessage: vi.fn(),
    createConfirm: vi.fn(),
  }),
}));

// Mock the router
vi.mock('/@/router', () => ({
  router: {
    currentRoute: {
      value: {
        query: {},
      },
    },
    replace: vi.fn(),
    addRoute: vi.fn(),
  },
}));

// Mock the permission store
vi.mock('/@/store/modules/permission', () => ({
  usePermissionStore: () => ({
    isDynamicAddedRoute: false,
    buildRoutesAction: vi.fn().mockResolvedValue([]),
    setDynamicAddedRoute: vi.fn(),
  }),
}));

// Mock the page enum
vi.mock('/@/enums/pageEnum', () => ({
  PageEnum: {
    BASE_HOME: '/dashboard',
    BASE_LOGIN: '/login',
  },
}));

// Mock the route constants
vi.mock('/@/router/routes/basic', () => ({
  PAGE_NOT_FOUND_ROUTE: {},
}));

// Mock the global settings
vi.mock('/@/hooks/setting', () => ({
  useGlobSetting: () => ({
    ctxPath: '/context',
  }),
}));

// Mock localStorage
const localStorageMock = {
  setItem: vi.fn(),
  getItem: vi.fn(),
  removeItem: vi.fn(),
  clear: vi.fn(),
};

Object.defineProperty(window, 'localStorage', {
  value: localStorageMock,
});

describe('store/modules/user', () => {
  const createUserInfo = (overrides: Partial<UserInfo> = {}): UserInfo => ({
    id: { id: 'user1' } as any,
    tenantId: { id: 'tenant1' } as any,
    customerId: { id: 'customer1' } as any,
    name: 'User One',
    email: 'user1@example.com',
    firstName: 'User',
    lastName: 'One',
    phone: '1234567890',
    authority: 'admin' as any,
    additionalInfo: {
      lastLoginTs: Date.now(),
      ...((overrides as any).additionalInfo || {}),
    },
    createdTime: Date.now(),
    ...overrides,
  });
  
  beforeEach(() => {
    vi.clearAllMocks();
    // Set up Pinia
    setActivePinia(createPinia());
  });

  describe('useUserStore', () => {
    it('should create user store', () => {
      const store = useUserStore();
      expect(store).toBeDefined();
    });

    it('should have initial state', () => {
      const store = useUserStore();
      expect(store.getUserInfo).toBeNull();
      expect(store.getToken).toBeUndefined();
      expect(store.getRefreshToken).toBeUndefined();
      expect(store.getAuthority).toBeUndefined();
      expect(store.getSessionTimeout).toBe(false);
      expect(store.getLastUpdateTime).toBe(0);
      expect(store.getPageCache).toEqual({});
      expect(store.getEmitter).toBeDefined();
    });

    it('should set token', () => {
      const store = useUserStore();
      const token: JwtPair = { token: 'test-token', refreshToken: 'test-refresh-token' };

      store.setToken(token);

      expect(store.getToken).toBe('test-token');
      expect(store.getRefreshToken).toBe('test-refresh-token');
      expect(localStorageMock.setItem).toHaveBeenCalledWith('jwt_token', 'test-token');
      expect(localStorageMock.setItem).toHaveBeenCalledWith('refresh_token', 'test-refresh-token');
    });

    it('should set session timeout', () => {
      const store = useUserStore();

      store.setSessionTimeout(true);

      expect(store.getSessionTimeout).toBe(true);
    });

    it('should set user info', () => {
      const store = useUserStore();
      const userInfo = createUserInfo();

      store.setUserInfo(userInfo);

      expect(store.getUserInfo).toEqual(expect.objectContaining(userInfo));
    });

    it('should set authority', () => {
      const store = useUserStore();
      const authority = 'admin';

      store.setAuthority(authority);

      expect(store.getAuthority).toBe('admin');
    });

    it('should reset state', () => {
      const store = useUserStore();

      store.resetState();

      expect(store.userInfo).toBeNull();
      expect(store.token).toBe('');
      expect(store.authority).toBeUndefined();
      expect(store.sessionTimeout).toBe(true);
    });

    it('should handle page cache', () => {
      const store = useUserStore();
      const key = 'testKey';
      const value = 'testValue';

      store.setPageCache(key, value);
      const cachedValue = store.getPageCacheByKey(key);

      expect(cachedValue).toBe('testValue');
    });

    it('should logout', async () => {
      const store = useUserStore();
      store.setToken({ token: 'test-token', refreshToken: 'test-refresh-token' });

      await store.logout();

      expect(store.getToken).toBeUndefined();
      expect(store.getSessionTimeout).toBe(true);
      expect(store.getUserInfo).toBeNull();
      expect(store.getAuthority).toBeUndefined();
    });

    it('should login successfully', async () => {
      const store = useUserStore();
      const loginParams = {
        username: 'testuser',
        password: 'testpass',
      };
      const jwtPair: JwtPair = { token: 'test-token', refreshToken: 'test-refresh-token' };
      const userInfo = createUserInfo();

      // Mock the API calls
      const loginApiMock = (await import('/@/api/tb/login')) as any;
      (loginApiMock.loginApi as any).mockResolvedValue(jwtPair);
      (loginApiMock.userInfoApi as any).mockResolvedValue(userInfo);

      const result = await store.login(loginParams);

      expect(loginApiMock.loginApi).toHaveBeenCalledWith({ username: 'testuser', password: 'testpass' }, undefined);
      expect(loginApiMock.userInfoApi).toHaveBeenCalled();
      expect(store.getToken).toBe('test-token');
      expect(store.getRefreshToken).toBe('test-refresh-token');
      expect(result).toEqual(userInfo);
    });

    it('should get user info', async () => {
      const store = useUserStore();
      const userInfo = createUserInfo();

      // Mock the API call
      const loginApiMock = (await import('/@/api/tb/login')) as any;
      (loginApiMock.userInfoApi as any).mockResolvedValue(userInfo);

      const result = await store.getUserInfoAction();

      expect(loginApiMock.userInfoApi).toHaveBeenCalled();
      expect(store.getUserInfo).toEqual(expect.objectContaining(userInfo));
      expect(result).toEqual(userInfo);
    });

    it('should handle login failure', async () => {
      const store = useUserStore();
      const loginParams = {
        username: 'testuser',
        password: 'wrongpass',
      };

      // Mock the API call to fail
      const loginApiMock = (await import('/@/api/tb/login')) as any;
      (loginApiMock.loginApi as any).mockResolvedValue(null);

      await expect(store.login(loginParams)).rejects.toThrow('Login failed, please try again!');

      expect(loginApiMock.loginApi).toHaveBeenCalledWith({ username: 'testuser', password: 'wrongpass' }, undefined);
    });
  });

  describe('useUserStoreWithOut', () => {
    it('should return user store', () => {
      const store = useUserStoreWithOut();
      expect(store).toBeDefined();
    });
  });

  describe('useEmitter', () => {
    it('should return emitter', () => {
      const emitter = useEmitter();
      expect(emitter).toBeDefined();
      expect(typeof emitter.on).toBe('function');
      expect(typeof emitter.emit).toBe('function');
    });
  });
});
