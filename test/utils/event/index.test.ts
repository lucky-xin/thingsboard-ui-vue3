...
describe('utils/event/index', () => {
  ...
  describe('resizeHandler', () => {
    ...
    it('should handle resize entries without listeners', () => {
      // Mock the resizeHandler function indirectly by triggering resize
      const mockElement = {
        __resizeListeners__: [],
      };
      ...
      expect(() => {
        const listeners = mockElement.__resizeListeners__ || [];
        if (listeners.length) {
    ...
    it('should handle resize entries with multiple listeners', () => {
      const listener1 = vi.fn();
      const listener2 = vi.fn();
      const mockElement = {
      ...
      // Execute the resizeHandler logic
      const listeners = mockElement.__resizeListeners__ || [];
      if (listeners.length) {
        listeners.forEach((fn: () => any) => {
          fn();
