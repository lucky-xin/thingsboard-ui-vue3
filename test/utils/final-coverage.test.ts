...
describe('Final Coverage Tests', () => {
  it('should test basic utility functions', async () => {
    ...
    const { formatToDate, formatToDateTime } = await import('/@/utils/dateUtil');
    expect(typeof formatToDate).toBe('function');
    expect(typeof formatToDateTime).toBe('function');
    
    const { buildUUID, buildShortUUID } = await import('/@/utils/uuid');
    expect(typeof buildUUID).toBe('function');
    expect(typeof buildShortUUID).toBe('function');
